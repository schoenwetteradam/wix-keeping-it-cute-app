...
export function resolveWixRedirectUri(fallbackRedirectUri = null) {
  ...
  // Strict validation: ensure no trailing slashes, correct protocol
  if (configuredRedirectUri) {
    try {
      const url = new URL(configuredRedirectUri);
      
      // Remove trailing slash if present (Wix is strict about this)
      let cleanUri = configuredRedirectUri;
      if (cleanUri.endsWith('/')) {
        cleanUri = cleanUri.slice(0, -1);
        console.warn(`Removed trailing slash from redirect URI: ${configuredRedirectUri} → ${cleanUri}`);
      }

      // Ensure HTTPS in production
      if (process.env.NODE_ENV === 'production' && url.protocol !== 'https:') {
        console.error(`WARNING: Redirect URI uses ${url.protocol} in production. Wix requires HTTPS.`);
      }

      // If we have a fallback, validate host match
      if (fallbackRedirectUri) {
        const fallbackUrl = new URL(fallbackRedirectUri);
        
        if (url.host !== fallbackUrl.host) {
          console.warn(
            `Configured Wix redirect URI (${cleanUri}) does not match current host (${fallbackRedirectUri}); using fallback to prevent invalid OAuth redirects.`
          );
  ...
  const resolvedRedirectUri = configuredRedirectUri || fallbackRedirectUri;
  ...
  return resolvedRedirectUri.endsWith('/') 
    ? resolvedRedirectUri.slice(0, -1) 
    : resolvedRedirectUri;
}
 ...
 */
// Wix OAuth Scopes - offline_access MUST be first for refresh tokens
const WIX_OAUTH_SCOPES = [
  // Core access - MUST be first
  'offline_access',
