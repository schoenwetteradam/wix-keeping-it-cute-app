...
export function resolveWixRedirectUri(fallbackRedirectUri = null) {
  ...
  if (configuredRedirectUri && fallbackRedirectUri) {
    try {
      const configuredHost = new URL(configuredRedirectUri).host;
      const fallbackHost = new URL(fallbackRedirectUri).host;

      if (configuredHost !== fallbackHost) {
        console.warn(
          `Configured Wix redirect URI (${configuredRedirectUri}) does not match current host (${fallbackRedirectUri}); using fallback to prevent invalid OAuth redirects.`
        );
